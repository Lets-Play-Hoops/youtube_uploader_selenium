...
def load_metadata(video_name: str, date_str: str) -> DefaultDict[str, str]:
    custom_dict = get_metadata(video_name, date_str)
    return defaultdict(str, custom_dict)
...
class YouTubeUploader:
    ...
    def __login(self):
        ...
        if self.browser.has_cookies_for_current_website():
            ...
            self.logger.debug(
                "Loaded cookies from {}".format(self.browser.cookies_folder_path)
            )
            time.sleep(Constant.USER_WAITING_TIME)
    ...
    def __upload(self) -> Tuple[bool, Optional[str]]:
        ...
        if edit_mode:
            ...
            while uploading_status_container is None:
                time.sleep(Constant.USER_WAITING_TIME)
                uploading_status_container = self.browser.find(
        ...
        title_field, description_field = self.browser.find_all(
            By.ID, Constant.TEXTBOX_ID, timeout=15
        )
        ...
        if playlist:
            ...
            self.logger.debug(
                'Playlist xpath: "{}".'.format(
                    Constant.PL_ITEM_CONTAINER.format(playlist)
                )
            )
            playlist_item = self.browser.find(
                By.XPATH,
                Constant.PL_ITEM_CONTAINER.format(playlist),
